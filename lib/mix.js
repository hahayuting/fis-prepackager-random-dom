...
var Mix = function (args){
    ...
    if (args) {
        this.init(args);
        this.run();
        // 提供给外部，来获取模块处理的key-value信息，用于调试检验
...
Mix.prototype.getMixJsString = function (js){
    ...
    while ((match = parser_2.exec(js)) !== null) {
        ...
        if (key == 'class') {
            ...
            passed = match[0].replace(new RegExp(match[3], "gi"), that.map[key][match[3]]);
        }
        js = js.replace(match[0], passed);
    }
    ...
    while ((match = parser_3.exec(js)) !== null) {
        ...
        matchList.forEach(function (matchStr) {
            if (matchStr.length > 0 && matchStr.indexOf('.') > -1) {
